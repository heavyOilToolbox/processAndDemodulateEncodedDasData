function [PD1, PD2, PD3, PD4, PD5, medianSignal] = StackProductDetectorsBasic(w, fc, fs, initialPhase, N, doPlot)
...
% 6. andreas' envelope detector
    ...
    try
        if (doPlot)
            hp = plot(dt(1:N), [PD1, PD2, PD3, PD4, PD5]);
        end
        medianSignal = median([PD1, PD2, PD3, PD4, PD5], 2);
        % medianSignalShaped = median([PD1f,PD2f,PD3f,PD4f,PD5f,PD6f], 2);
    catch
        if (doPlot)
            hp = plot(dt(1:N), [PD1, PD2, PD3, PD4]);
        end
        medianSignal = median([PD1, PD2, PD3, PD4], 2);
